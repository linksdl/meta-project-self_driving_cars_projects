...
def project_detections_into_bev(bev_map, detections, configs, color=[]):
    ...
    for row in detections:
        ...
        cv2.line(bev_map, (31, 237), (50, 237), (255, 255, 0), 2)
...
def validate_object_labels(object_labels, pcl, configs, min_num_points):
    ...
    vehicle_to_labels = [np.linalg.inv(waymo_utils.get_box_transformation_matrix(label.box)) for label in object_labels]
    vehicle_to_labels = np.stack(vehicle_to_labels)

    # strip away intensity information from point cloud
    pcl_no_int = pcl[:, :3]
    # convert pointcloud to homogeneous coordinates
    pcl1 = np.concatenate((pcl_no_int, np.ones_like(pcl_no_int[:, 0:1])),axis=1)
    ...
    # for each pair of LIDAR point & label, check if point is inside the label's box (mask shape is [label,
    # LIDAR point])
