...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():
        ...
        if 'darknet' in configs.arch:
            ...
            # outputs['hm_cen'] = np.sigmoid(outputs['hm_cen'])
            # outputs['cen_offset'] = np.sigmoid(outputs['cen_offset'])
    ...
    if len(detections) > 0:
        ...
        for det in detections:
            class_id, bev_x, bev_y, z, h, bev_w, bev_l, yaw = det
